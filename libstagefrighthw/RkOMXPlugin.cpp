...
RkOMXPlugin::RkOMXPlugin()
    ...
    if (mLibHandle != NULL) {
        ...
        LOGI("Calling OMX_Init...");
        (*mInit)();
	LOGI("RkOMXPlugin Created.");
    }else{
        LOGE("Unable to open libomx_sharedlibrary.so");
    }
}
...
OMX_ERRORTYPE RkOMXPlugin::makeComponentInstance(
    ...
    LOGI("makeComponentInstance: name=%s", name);
    if (mLibHandle == NULL) {
...
OMX_ERRORTYPE RkOMXPlugin::getRolesOfComponent(
    ...
    LOGI("getRolesOfComponent: name=%s", name);
    roles->clear();
